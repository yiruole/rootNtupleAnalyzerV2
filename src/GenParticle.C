...
float & GenParticle::Pt       (){ return m_collection -> GetData() -> GenPart_pt [m_raw_index]; }
float & GenParticle::Eta      (){ return m_collection -> GetData() -> GenPart_eta[m_raw_index]; } 
float & GenParticle::Phi      (){ return m_collection -> GetData() -> GenPart_phi[m_raw_index]; } 
float & GenParticle::Mass     (){ return m_collection -> GetData() -> GenPart_mass[m_raw_index]; } 

// ID variables		                                                       

int    GenParticle::PdgId       (){ return m_collection -> GetData() -> GenPart_pdgId           [m_raw_index]; }
int    GenParticle::MotherIndex (){ return m_collection -> GetData() -> GenPart_genPartIdxMother[m_raw_index]; }
int    GenParticle::Status      (){ return m_collection -> GetData() -> GenPart_status          [m_raw_index]; }
