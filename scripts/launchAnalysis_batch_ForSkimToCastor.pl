...
## and copy the skim output to castor    ##
###########################################
...
if($opt_d) {$castorDir = $opt_d;}

system "mkdir -p $outputDir";
system "cp $cutfile $outputDir/";

#create CASTOR dir
my $currentPath = $ENV{'CASTOR_HOME'};
my $fullCastorPath = $currentPath."/".$castorDir;
...
#my @directories = split(/\//, $castorDir);
...
#    system "./main $line $cutfile $treename $outputDir/$codename\_\_\_$dataset $outputDir/$codename\_\_\_$dataset";
    ...
    print "./scripts/submit_batch_ForSkimToCastor.py -i $line -c $cutfile -t $treename -o $outputDir/$codename\_\_\_$dataset -n $njobs -q $queue -d $fullCastorPath\n";
    system "./scripts/submit_batch_ForSkimToCastor.py -i $line -c $cutfile -t $treename -o $outputDir/$codename\_\_\_$dataset -n $njobs -q $queue -d $fullCastorPath";    
...
sub help(){
    print "Usage: perl ./script/launchAnalysis.pl -i <inputList> -c <cutfile> -n <treename> -o <outputDir> -j <njobs> -q <queue> -d <castorDir> [-w <wait> -h <help?>] \n";
    print "Example: perl scripts/launchAnalysis_batch.pl -i HeepStudies_v1/inputListAllCurrent.txt -c HeepStudies_v1/cutFile_HeepElectronStudiesV1.txt -n rootTupleTree/tree -o TestFrancesco -j 2 -q 1nh -w 5 -d LQ/Ntuples \n";
    print "Options:\n";
    ...
    print "-d <castorDir>:      castor directory for the skim output (specify only the last part of the directory path, after $CASTOR_HOME)\n";
