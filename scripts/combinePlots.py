...
for sample,pieceList in dictSamples.iteritems():
    ...
    for lin in open( options.inputList ):
        ...
        # prepare to combine
        print '\tfound matching dataset:',combineCommon.SanitizeDatasetNameFromInputList(dataset_fromInputList) + " ... ",
        #print combineCommon.SanitizeDatasetNameFromInputList(dataset_fromInputList),dataset_fromInputList,
        sys.stdout.flush()
        ...
        #---Calculate weight
        Ntot = float(data[0]['N'])
        weight,plotWeight,xsection_X_intLumi,sumAMCatNLOweights = CalculateWeight(Ntot,xsection_val,options.intLumi,inputRootFile)
