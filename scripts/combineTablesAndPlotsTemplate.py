...
for lin in open(options.inputList):
    ...
    xsection_val = combineCommon.lookupXSection(
        combineCommon.SanitizeDatasetNameFromInputList(dataset_fromInputList),
        xsectionDict,
    )
...
for lin in open(options.inputList):
    ...
    # strip off the slashes and the .txt at the end
    # so this will look like 'TTJets_DiLept_reduced_skim'
    print combineCommon.SanitizeDatasetNameFromInputList(
        dataset_fromInputList
    ) + " ... ",
    # print combineCommon.SanitizeDatasetNameFromInputList(dataset_fromInputList),dataset_fromInputList,
    sys.stdout.flush()
...
if options.qcdClosure:
    ...
    nonQCDBkgTable = dictFinalTables[nonTTbarAMCBkgSampleName]
...
if not options.tablesOnly:
    ...
    for histDict in dictFinalHisto.itervalues():  # for each sample's dict
        for (
            histo
        ) in histDict.itervalues():  # for each hist contained in the sample's dict
