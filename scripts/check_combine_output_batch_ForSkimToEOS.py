...
usage = "usage: %prog [options] \nExample: \n./scripts/check_combine_output_batch.py -i `pwd`/HeepStudies_v1/QCDEMEnriched/inputList2030.txt -c analysisClass_HeepElectronStudiesV1 -d `pwd`/HeepStudies_v1/QCDEMEnriched -o `pwd`/HeepStudies_v1/QCDEMEnriched -q 1nh -s /eos/cms/store/user/eberry/"
...
parser.add_option("-s", "--eosDir", dest="eosDir",
                  help="eos directory where the skim output is located (full path required)",
                  metavar="EOSDIR")
...
for n, lin in enumerate( open( options.inputList ) ):
    ...
    for script in list_scriptBatchDir:
        ...
        skimFile = eosBatchDir + "/" + dataset_mod + "_" + number + "_1_rsk.root"

        resubmitThisJob = False

        # Check that for each .src you have the 4 correspondent .log, .root, .dat and skim.root files
        if(os.path.isfile(logFile) == False or os.path.isfile(rootFile) == False or os.path.isfile(datFile) == False or os.system("/afs/cern.ch/project/eos/installation/pro/bin/eos.select ls -l " + skimFile)!=0 ):
            resubmitThisJob = True
        else:
            for line in open(logFile):
                if ( "error" in line.lower() and 
                     "Error checking device name: LABEL" not in line and 
                     "Xrd:"                              not in line and 
                     "unknown branch"                    not in line and 
                     "Phi_mpi_pi"                        not in line and 
                     "Error accessing path/file"         not in line and 
                     "PassLogErrorTooManyClusters"       not in line ):
                       resubmitThisJob = True
